...
#generate list of unique ECFPs in the test set
ECFP_unique_t <- unique(ECFP) #all unique ECFPs
ECFP_number_t <- length(ECFP_unique_t) #number of unique ECFPs
...
match <- data.frame(
  value = c(matched, unmatched), cat = c('matched','unmatched')) %>%
  mutate(cat = as.character(cat))

#add unique ECFPs
ec <- data.frame(
  value = c(ECFP_number/1000, ECFP_number_t/1000), cat = as.character(c('trainset','testset')))
match <- bind_rows(match, ec) %>%
  mutate(fig = as.character(c('Testset','Testset',
                              'Features','Features'))) %>%
  mutate(cat = as.factor(cat))


p <- ggplot(data = filter(match, fig == 'Features'), aes(x = cat, y = value))

p +
  geom_bar(stat = 'identity', fill = 'blue') +
  ylab('Total number of unique ECFPs [10^3]') +
  xlab('')

```

Figure: Total number of unique ECFPs (x 1000) in the test set (left) and in the training set (right). While the number of unique structural features was comparable in both data sets, about 30% of features in the test set did not appear in the training set (see figure below).

```{r, echo=FALSE, message=FALSE}
p <- ggplot(data = filter(match, fig == 'Testset'), aes(x = cat, y = value))

p +
  geom_bar(stat = 'identity', fill = 'blue') +
  ylab('% ECFPs in testset') +
  xlab('')
```

Figure: This graph shows the proportion of ECFPs in the test set that were successfully matched to the ECFPs in the training set. Almost 30% of the structural features in the test set were different from those in the training set suggesting significant differences in the molecular structures between the compounds of the two data sets.

##Results from the machine learning application
###ROC and Precision Recall Plots
###*AUC Summary*
The following table summarizes the ROC AUC score for each phenotype prediction made by the random forest machine learning algorithm. The P2, P7, and P8 phenotypes are "death" indicators which indicate that the zebrafish embryo died.  Death indicators P7 and P8 had the highest 2 AUC scores and P2 was 5th highest.
...
## ROC Curves
The following plot shows the ROC curves for the death phenotypes.  The curves show that the random forest prediction model does a reasonable job of predicting the death indicators from chemical fingerprints in the training data.  As indicated in the AUC table above, P7, P8, and P14 were predicted best by the random forest model.
...
The following plot shows the precision recall curves for death indicator phenotypes.
...
v20.meta.per_compound_desc <- "File containing experiment ID and cell line ID that is joined to the EC50 data set."
...
The machine learning model generated and verified using the zebrafish model is used to predict drug efficacy on cancer cells. Although multiple phenotypes of zebra fish were used to generate the model, to simplify the study, only life/death is assessed for the cancer cells. Efficacy of compounds in killing cancer cells was predicted, the predicted data were evaluated by comparing to true experimental data showing efficacy, reflected by EC50, which is the amount of drug needed to kill half of the cells.
        The "Predictions.csv" file summarized predicted probability of compounds being efficacious on causing several phenotypes of cancer cells and their impact on cancer cell viability. Because the majority of the probability numbers are very small (smaller than 0.5), to choose the best response to work with, the number of positive indicators were compared among those phenotypes. We determined that P11.true, which is the probability of the compounds being able to kill cancer cells, was the best predictor to be compared with the true experimental data. This will show whether the prediction model generated using the zebra fish model could be used on mammalian cells.
...
The prediction data of the efficacy of the compounds using the zebrafish model is "predictions.csv", in which the efficacy of the drugs is reflected by probability values under column names "Px.true" where x is numbers 1-14 representing the prediction on different responses.
```{r, echo=FALSE, message=FALSE}
...
The scores are plotted for each cell line:
```{r, echo=FALSE, message=FALSE}
library(ggplot2)
...
In fact, even if the predicted efficacy data showed good balance between positive and negative values, the model could just not work on the cancer cells. From biology understanding, zebra fish and cancer cells are too different from each other in terms of phenotypes and tolerance to the compounds. There is a high chance that the phenomena observed from zebra fish won't translate well to mammalian cancer cell lines.

However, one potential drawback of the current project is that we are only looking at life/death of the cancer cells. There could be other phenotypes of the cells that were able to be predicted using the zebra fish model. Given the short amount of time for the project, we did not have a chance to evaluate other possibilities but this could be something to explore further.

##Conclusions:
We established a machine learning framework to predict the physiological effects of an unknown drug on zebrafish embryo development. A data set consisting of more than 4,000 manual observations of 10 distinct physiological phenotypes was used to train a random forest classification algorithm. The achieved scores between 0.7 and 0.8 for some phenotypes suggest that the model may be used on much larger libraries with millions of compounds. We tested if our model could predict cell death in human cancer cells that were exposed to 545 different drugs. Physical and structural features were generated for these drugs and matched with the same features in the training set. Life/death predictions were then obtained from the random forest model. However, we found that the predicted outcomes did not compare well to the experimental result even when we varied the decision thresholds. There are several reasons why the zebrafish model failed to predict cell death on cancer cells. One important possibility is that the cancer compounds had physical and structural properties that were very distinct from those that were used for training the model. Expanding the training set to include more compounds with similar molecular features would most likely improve the accuracy of the model. Nevertheless, the results of the zebrafish model are encouraging. This model provides a resource of compound-activity associations in a vertebrate and demonstrates a means for combined in vivo/in silico screening with potentially immediate clinical relevance.
